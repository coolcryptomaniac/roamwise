...
test('business page assets stay within a 25 KiB gzip budget and never load in the main app', () => {
  ...
  const htmlSource = fs.readFileSync(path.join(root, 'business/index.html'), 'utf8');
  const files = ['business/index.html', ...Array.from(htmlSource.matchAll(/(?:src|href)="(\.\.\/features\/business-travel\/[^"]+\.(?:js|css))"/g), m => path.relative(root, path.resolve(root, 'business', m[1])))];
  ...
  assert.ok(main.includes('href="business/"')); assert.doesNotMatch(main, /<script[^>]+(?:business\/|features\/business-travel\/)/);
