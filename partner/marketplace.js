...
function productionShell(){
  ...
  if(!$('#rwMarketTrust'))$('.hero').insertAdjacentHTML('beforeend','<div id="rwMarketTrust" class="rw-market-trust"><span><b>5–7%</b> host commission</span><span><b>₹0</b> guest fee</span><span><b>Verified</b> before listing</span><span><b>Direct</b> host confirmation</span></div>');
...
function submitVerifiedOwner(event){
  ...
  (async function(){
    ...
    var doc={email:u.email||'',ownerName:String(($('#oname')||{}).value||'').trim(),ownerWa:String(($('#ophone')||{}).value||'').trim(),name:String(($('#oprop')||{}).value||'').trim(),country:String(($('#ocountry')||{}).value||'').trim(),zone:String(($('#ozone')||{}).value||'').trim(),area:String(($('#oarea')||{}).value||'').trim(),type:String(($('#otype')||{}).value||''),roomCount:Math.max(1,Number(($('#orooms')||{}).value||1)),startPrice:Math.max(1,Number(($('#oprice')||{}).value||1)),highSeasonMonths:String(($('#opeak')||{}).value||'').split(',').map(function(x){return x.trim().slice(0,12)}).filter(Boolean).slice(0,12),requestedPlan:String(($('#oplan')||{}).value||'free'),website:cleanHttp(($('#oweb')||{}).value),hook:String(($('#ohook')||{}).value||'').trim(),status:'pending',verification:{ownerAttestation:true,rateAttestation:true,walkthroughConsent:true,publicLocationUrl:cleanHttp(($('#rwPublicLocation')||{}).value),identity:'pending',property:'pending',payout:'pending',overall:'pending'},bookingPolicy:{requestToBook:true},updatedAt:now()};
    if(!doc.ownerName||!doc.name||!doc.country||!doc.zone){alert('Add owner name, property name, country and city.');return}
...
async function approvePartner(uid){
  ...
  await ref.set({status:'active',verified:true,verifiedState:'verified',commercialPlan:'free',commissionPct:7,approvedAt:now(),verification:Object.assign({},v,{identity:'verified',property:'verified',overall:'verified',reviewedAt:now()})},{merge:true});
