...
test('proposal and creators share the RoamWise campus design system', () => {
  for (const html of [proposal, creators]) {
    assert.match(html, /<meta name="theme-color" content="#07090F">/);
    assert.match(html, /href="\/nmims\/campus-theme\.css\?v=\d+"/);
    assert.match(html, /family=Outfit/);
  }
  for (const color of ['#07090f', '#c4302b', '#e8ba6c', '#16bf96', '#bf8cff']) {
    assert.ok(css.toLowerCase().includes(color), `missing site brand color ${color}`);
  }
  assert.match(css, /--rw-accent-sunset:#f46f64/i);
  assert.match(css, /--rw-accent-amber:#f6b26e/i);
});

test('motion respects accessibility and leaves recipient and creator flows unchanged', () => {
  ...
  assert.match(proposal, /organisers or participating students/i);
  assert.match(proposal, /only one referral bonus/i);
});
