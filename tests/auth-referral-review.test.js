// Validate the full hardened rules, whether still a candidate or canonical.
...
test('full Firestore rules preserve referral, partner and payment integrity',()=>{
  ...
  try{
    ...
    // Prior to release, generate a candidate. Once canonical rules have been
    // hardened, validate that source directly instead of transforming twice.
    if(!before.startsWith('// 2026-09-16: NMIMS proposed, not live.')){
      execFileSync('python3',[path.join(root,'tools/prepare-auth-referral-rules.py'),'--input',path.join(root,'firestore.rules'),'--output',output],{cwd:root});
    }else fs.writeFileSync(output,before);
