...
const start=source.indexOf('function rwSendVerificationAndSignOut(');
...
function harness({create,signIn,send,verified=false}={}){
  ...
  const ctx={firebase,window:{RWAuthSecurity:{normaliseEmail:e=>e.trim().toLowerCase(),passwordStatus:()=>({ok:true}),actionCodeSettings:()=>({url:'https://www.roamwise.co.in/?auth=verified'}),rememberProvider:()=>{}}},AUTH_READY:true,authMode:'up',rwEmailAuthBusy:false,
    el:id=>elements[id]||(elements[id]={}),authError:m=>messages.push(m),rwShowVerificationPane:(e,m)=>messages.push(m),rwIsUnverifiedPasswordUser:u=>!u.emailVerified,rwSetAuthBusy:()=>{},rwApplyAuthModeUI:()=>{},showToast:m=>messages.push(m),closeAuth:()=>{},track:()=>{},rwRefActive:()=>null,lsSet:()=>{},pendingVerificationEmail:'',friendly:e=>String(e&&e.code||'').includes('email-already-in-use')?'Account exists — choose sign in or Google.':String(e&&e.code||'')};
...
test('new account: verification accepted and one UID retained',async()=>{
  let n=0;
  const h=harness({create:()=>{n++;return Promise.resolve({user:h.user});},signIn:()=>{throw Error('unexpected sign in');},send:()=>Promise.resolve()});
  await h.run();assert.equal(n,1);assert.deepEqual(h.signs,['out']);assert.match(h.messages.join(' '),/accepted the request/);assert.doesNotMatch(h.messages.join(' '),/could not send/);
});
test('failed verification send never deletes account or asserts email delivery',async()=>{
  ...
  await h.run();assert.deepEqual(h.signs,['out']);assert.match(h.messages.join(' '),/could not send/);assert.doesNotMatch(h.messages.join(' '),/Delivery is not guaranteed/);
});
test('duplicate signup with same password resumes original UID and resends without new signup',async()=>{
  let signin=0,send=0;
  ...
  await h.run();assert.equal(signin,1);assert.equal(send,1);assert.equal(h.user.uid,'original-uid');assert.deepEqual(h.signs,['out']);
});
...
test('invalid continue domain retries once with default Firebase action URL',async()=>{
  ...
  await h.run();assert.equal(calls,2);assert.match(h.messages.join(' '),/accepted the request/);
});
