...
| `FIREBASE-AUTH-HARDENING.md` | Console and release runbook for provider-safe login, password policy, branded email actions, App Check, referral-rule deployment, founder-seat reconciliation, and the phone-OTP decision. | current | Before deploying or changing Firebase Authentication, App Check, email templates, login UI, referrals, or the public founder counter. |
...
| `YOGA-WELLNESS-PARTNER-PROGRAM.md` | Self-serve program for yoga instructors/studios/retreat hosts: reuses the `CREATOR-OUTREACH.md` 25%→30%→lifetime-Pro ladder unchanged, adds a bookable Certified Experience listing (`experiences-data.js` schema, new `tier:'wellness'` value, 12% commission) as a second distribution channel, and explicitly declines to invent a new free-Pro perk. | current | Before touching `/yoga/`, the `crm` collection's `seg:'yogapreneur'` create rule, or the Certified Experience catalog's `wellness` tier. |
